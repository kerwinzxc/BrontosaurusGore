...
{
    //float bloomCutoff = 0.8f;
    float3 resource = renderPackage.SampleLevel(Sampler, input.tex, 0).rgb;

    float3 relativeLuminance = (float3)0;

    relativeLuminance.r = 0.3f; // 0.2126f;
    relativeLuminance.g = 0.59f;// 0.7152f;
    relativeLuminance.b = 0.11f;// 0.0722f;
    ...
    PixelOutput output;
    output.color.rgb = luminanceLenghto.xxx;
    output.color.a = 1.0f;    
    return output;
}
